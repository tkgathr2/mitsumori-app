...
単価の**正は管理画面 /admin（DBの price_companies）だけ**。
2026-07-17 の社長決定により、**Googleスプレッドシートとの通信は全経路を撤去**した
（サービスアカウント直読み／price_cache／公開CSV／GASの受け口 `/api/prices/update`／シードAPI `/api/admin/seed`）。
単価を変えるときは管理画面で編集する。シートは見ない。

- 取得経路（`lib/prices-server.ts`）: `price_companies` のみ（最大60秒メモリキャッシュ）
- **DBから会社が取れないときはスナップショットへフォールバックしない**。空の `companies` を返し、
  画面は「単価を取得できませんでした。管理画面で単価を登録してください」を表示する。
  （スナップショットの会社単価は残業が0円で、フォールバックすると間違った金額が静かに客先へ出るため。
  間違った金額を出すより出さない方が安全、という設計判断）
- 資器材（カラーコーン等）は静的ファイル `data/prices-snapshot.json` 由来＝シート通信ではない。
  `snapshotData()` は資器材とその取得日だけを返す（会社は返さない）。
...
| `GET /api/prices` | 単価データ（管理画面の単価マスタ `price_companies` のみ。取得不可なら `companies` は空） |
